    ...
    with chat_container:
        ...
        if not messages_src:
            ...
            for i, message in enumerate(messages_src):
                ...
                # Event messages have an `event_type` and sometimes extra fields like `options`.
                if mtype == 'event':
                    event_type = message.get('event_type', 'event')
                    # Show special rendering for some known event types
                    if event_type == 'show_reel_options':
                        st.markdown(f"**🔔 {event_type}:** {content}")
                        options = message.get('options') or []
                        if options:
                            for opt in options:
                                st.markdown(f"- `{opt}`")
                    elif event_type == 'tool_calling':
                        st.markdown(f"**🛠️ {event_type}:** {content}")
                    else:
                        st.markdown(f"**🔔 {event_type}:** {content}")

                # Standard role-based rendering
                elif role == 'user':
        ...
        if submit and message:
            ...
            with st.spinner("AI is thinking..."):
                ...
                if streaming_response:
                    ...
                    # NOTE: client-side summary logs removed; raw streaming chunks
                    # are logged directly in `APIClient.process_streaming_response`
                    # when `debug_mode` and `log_raw_streaming` are enabled.
                else:
